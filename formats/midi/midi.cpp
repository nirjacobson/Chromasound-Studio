...
QList<QList<Track::Item*> > MIDI::toTracks(const MIDITrack& midiTrack, const int division)
{
    QList<QList<Track::Item*>> tracks;
    ...
    for (int c = 0; c < 16; c++) {
        ...
        for (int i = 0; i < midiTrack.events(); i++) {
            ...
            try {
                ...
                if (midiEvent.isKeyOn(c) && midiEvent.data2() != 0) {
                ...
                } else if (midiEvent.isKeyOff(c) || (midiEvent.isKeyOn(c) && midiEvent.data2() == 0)) {
                    if (notes.contains(midiEvent.data1())) {
