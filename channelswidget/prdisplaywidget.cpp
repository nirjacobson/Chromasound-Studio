...
void PRDisplayWidget::paintEvent(QPaintEvent* event)
{
    ...
    float length = _app->project().getPattern(_app->activePattern()).getLength();
    ...
    QList<Track::Item*>& items = _app->project().getPattern(_app->activePattern()).getTrack(_index).items();
    ...
    if (_app->playMode() == Application::PlayMode::Song) {
        ...
        if (activePatterns.contains(_app->activePattern())) {
            position = _app->position() - activePatterns[_app->activePattern()];
        } else {
            position = 0;
        }
    }
