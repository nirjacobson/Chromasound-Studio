...
blargg_err_t Gme_Info_::set_sample_rate_( long )            {
    return 0;
}
void         Gme_Info_::pre_load()                          {
    Gme_File::pre_load();    // skip Music_Emu
}
void         Gme_Info_::post_load_()                        {
    Gme_File::post_load_();    // skip Music_Emu
}
...
blargg_err_t Gme_Info_::start_track_( int )                 {
    return "Use full emulator for playback";
}
blargg_err_t Gme_Info_::play_( long, sample_t* )            {
    return "Use full emulator for playback";
}
