#include "fm2envelopedisplaywidget.h"

QPointF FM2EnvelopeDisplayWidget::NULL_POINTF = QPointF(-1, -1);

FM2EnvelopeDisplayWidget::FM2EnvelopeDisplayWidget(QWidget *parent)
    : QWidget{parent}
    , _settings(nullptr)
...
void FM2EnvelopeDisplayWidget::setSettings(const FM2EnvelopeSettings* settings)
{
    _settings = settings;
    update();
}

QList<QPointF> FM2EnvelopeDisplayWidget::getPoints() const
{
    ...
    if (_settings->arf() > 0) {
        ...
        if (_settings->type() == FM2EnvelopeSettings::EGType::Percussive) {
            if (_settings->rrf() > 0) {
                p3.setY(1);
                if (_settings->rrf() == 1) {
                    p3.setX(p2.x());
...
const QColor& FM2EnvelopeDisplayWidget::backgroundColor() const
{
    return _backgroundColor;
}

const QColor& FM2EnvelopeDisplayWidget::borderColor() const
{
    return _borderColor;
}

const QColor& FM2EnvelopeDisplayWidget::envelopeColor() const
{
    return _envelopeColor;
}

const QColor& FM2EnvelopeDisplayWidget::levelColor() const
{
    return _levelColor;
}

void FM2EnvelopeDisplayWidget::setBackgroundColor(const QColor& color)
{
    _backgroundColor = color;
}

void FM2EnvelopeDisplayWidget::setBorderColor(const QColor& color)
{
    _borderColor = color;
}

void FM2EnvelopeDisplayWidget::setEnvelopeColor(const QColor& color)
{
    _envelopeColor = color;
}

void FM2EnvelopeDisplayWidget::setLevelColor(const QColor& color)
{
    _levelColor = color;
}

void FM2EnvelopeDisplayWidget::paintEvent(QPaintEvent* event)
{
