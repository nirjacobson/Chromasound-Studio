...
void OPLImportDialog::load(const QString& path)
{
    ...
    while (!in.atEnd() && command != 0x66) {
        ...
        if (command == 0x51 && aa >= 0x20 && aa <= 0x28 && (dd & 0x10) > 0 && (ym2413[aa + 0x10] >> 4) == 0) {
            ...
            // create OPLSettings from memory
            OPLSettings channelSettings;

            quint8 datum = ym2413[0x00];
            channelSettings.operators()[0].setAm(datum & (1 << 7));
            channelSettings.operators()[0].setVib(datum & (1 << 6));
            channelSettings.operators()[0].envelopeSettings().setType(static_cast<OPLEnvelopeSettings::EGType>(datum & (1 << 5)));
            ...
            channelSettings.operators()[1].envelopeSettings().setType(static_cast<OPLEnvelopeSettings::EGType>(datum & (1 << 5)));
...
int OPLImportDialog::ensurePatch(const OPLSettings& settings)
{
...
void OPLImportDialog::sendTriggered()
{
    ...
    _app->undoStack().push(new EditOPLSettingsCommand(_app->window(), _app->project().userTone(), _patchSettings[selectedIndex]));
}
